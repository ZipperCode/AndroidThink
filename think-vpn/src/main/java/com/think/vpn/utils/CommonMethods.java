...
public class CommonMethods {
    ...
    public static boolean ComputeIPChecksum(IPHeader ipHeader) {
        int oldCrc = ipHeader.getCheckSum();
        ipHeader.setCheckSum((short) 0);
        short newCrc = CommonMethods.checksum(0, ipHeader.mHeaderBuffer.array(), ipHeader.mDataOffset, ipHeader.getHeaderLength());
        ipHeader.setCheckSum(newCrc);
    ...
    public static boolean ComputeTCPChecksum(IPHeader ipHeader, TCPHeader tcpHeader) {
        ...
        int ipData_len = ipHeader.getTotalLen();
        if (ipData_len < 0) {
            return false;
        }

        long sum = getsum(ipHeader.mHeaderBuffer.array(), ipHeader.mDataOffset + IPHeader.SRC_ADDRESS_OFFSET, 8);
        sum += ipHeader.getProtocol() & 0xFF;
        sum += ipData_len;

        int oldCrc = tcpHeader.getCheckSum();
        tcpHeader.setCheckSum((short) 0);

        short newCrc = checksum(sum, tcpHeader.bytes(), tcpHeader.mDataOffset, ipData_len);
    ...
    public static boolean ComputeUDPChecksum(IPHeader ipHeader, UDPHeader udpHeader) {
        ...
        udpHeader.setCheckSum((short) 0);

        short newCrc = checksum(sum, udpHeader.data().array(), udpHeader.offset(), ipData_len);
